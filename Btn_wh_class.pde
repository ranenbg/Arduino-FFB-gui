class WheelButton {
  ...
  void show(int i, boolean resize) {
    if (resize) {
      s = min(wScaleX, wScaleY)*btn_size_init;
      float d = min(wScaleX, wScaleY)*(btn_size_init+btn_sep_init);
      if (i <= 7) {
        x = 0.05*widthprev +i*d;
        y = heightprev-posY*1.85;
      } else if (i > 7 && i < 16) {
        x = 0.05*widthprev +(i-8)*d;
        y = heightprev-posY*1.85+d;
      } else if (i > 15 && i < 24) {
        x = 0.05*widthprev +(i-16)*d;
        y = heightprev-posY*1.85+2*d;
      }
    }
    ...
    if (wScaleX <= 1.0 && wScaleY <= 1.0) {
      if (i<=9) {
        text(i, 0.5*font_size, 1.1*font_size);
      } else {
        text(i, 0.15*font_size, 1.1*font_size);
      }
    } else {
      text(i, 0.5*(s - textWidth(str(i))), 1.1*font_size);
    }
    popMatrix();
  }
}
